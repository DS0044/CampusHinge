 ...
 */
export async function resetSwipes(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
 ...
 */
export async function resetAllSwipes(_req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    await db.query(`DELETE FROM swipes`);
    await db.query(`DELETE FROM matches`);
    await db.query(`DELETE FROM notifications`);
...
export default { resetSwipes, resetAllSwipes };
