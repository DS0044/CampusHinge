...
export async function signup(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const { rows } = await db.query<{ id: string; is_banned: number | boolean }>(
...
export async function verifyOtp(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const { rows: profileRows } = await db.query<{ id: string; photos: string | string[] }>(
...
export default { signup, login, verifyOtp, resendOtp, googleAuth };
