...
interface GatedProfile extends Profile {
  is_locked?: boolean;
  has_matched?: boolean;
  match_id?: string;
  real_name?: string;
...
interface GatedProfileModalProps {
  targetUserId: string | null;
  onClose: () => void;
  onMatchCreated?: () => void;
}

export default function GatedProfileModal({ targetUserId, onClose, onMatchCreated }: GatedProfileModalProps): React.ReactNode {
  const navigate = useNavigate();
  const [profile, setProfile] = useState<GatedProfile | null>(null);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string>('');
  const [swiping, setSwiping] = useState<boolean>(false);
  const [unlocking, setUnlocking] = useState<boolean>(false);
  const [matchInfo, setMatchInfo] = useState<{ matchId: string } | null>(null);
      ...
      try {
        ...
        try {
          ...
          if (mounted && me?.interests) {
            const list: string[] = Array.isArray(me.interests)
              ? me.interests
        ...
        if (mounted) {
          const prof = (res.data as any)?.profile || null;
          setProfile(prof);
  ...
  async function loadGatedProfile(silent = false): Promise<void> {
    ...
    try {
      ...
      if (!silent) setError(err.message || 'Error reloading profile');
    } finally {
      if (!silent) setLoading(false);
    }
  }

  async function handleLikeBack(): Promise<void> {
    if (!targetUserId) return;
    setSwiping(true);
    setError('');
    try {
