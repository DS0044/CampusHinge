...
export default function NotificationsPage() {
  ...
  async function loadNotifications(silent = false) {
    ...
    try {
      ...
      const resData = res.data as { notifications?: AppNotification[] } | AppNotification[] | undefined;
      const list = (resData && !Array.isArray(resData) && resData.notifications) ? resData.notifications : (Array.isArray(resData) ? resData : []);
      setNotifications(list as AppNotification[]);
    } catch (err: unknown) {
      if (!silent) setError(err instanceof Error ? err.message : 'Failed to load notifications');
    } finally {
  ...
  async function handleLikeBack(e: React.MouseEvent, notif: AppNotification) {
    e.stopPropagation();
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to match back.');
    } finally {
      setLikingId(null);
    }
  }
