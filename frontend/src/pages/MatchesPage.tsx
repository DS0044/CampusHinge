...
export default function MatchesPage() {
  ...
  useEffect(() => {
    ...
    const unsubMessage = onSocketEvent('new_message', (rawMsg: unknown) => {
      const msg = rawMsg as { match_id: string; content: string; created_at: string };
      setMatches((prev) =>
        prev.map((m) => {
  ...
  async function loadMatches() {
    ...
    try {
      ...
      const resData = res.data as { matches?: MatchItem[] } | MatchItem[] | undefined;
      const list = (resData && !Array.isArray(resData) && resData.matches) ? resData.matches : (Array.isArray(resData) ? resData : []);
      setMatches(list as MatchItem[]);
    } catch (err: unknown) {
