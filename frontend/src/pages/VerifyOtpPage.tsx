...
const COOLDOWN_DURATION: number = 30;
...
function getRemainingSeconds(cleanEmail: string): number {
...
export default function VerifyOtpPage(): React.ReactNode {
  ...
  const [remainingSeconds, setRemainingSeconds] = useState<number>(() => getRemainingSeconds(cleanEmail));

  const isLogin = Boolean((location.state as any)?.isLogin);
  ...
  async function handleResend(e?: React.MouseEvent): Promise<void> {
